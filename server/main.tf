...
resource "alicloud_vswitch" "rabbitmq" {
  count = "${length(var.vswitch_cidr_rabbitmq)}"
  name = "vswitch_for_rabbitmq${var.prefix}-${count.index}"
  vpc_id = "${alicloud_vpc.default.id}"
  cidr_block = "${element(var.vswitch_cidr_rabbitmq, count.index)}"
  availability_zone = "${lookup(data.alicloud_zones.default.zones[count.index%length(data.alicloud_zones.default.zones)], "id")}"
}

resource "alicloud_vswitch" "db_service" {
  count = "${length(var.vswitch_cidr_db_service)}"
  name = "vswitch_for_db_service${var.prefix}-${count.index}"
  vpc_id = "${alicloud_vpc.default.id}"
  cidr_block = "${element(var.vswitch_cidr_db_service, count.index)}"
  availability_zone = "${lookup(data.alicloud_zones.default.zones[count.index%length(data.alicloud_zones.default.zones)], "id")}"
}
...
resource "alicloud_snat_entry" "cf_0"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.cf.0.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "cf_1"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.cf.1.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "cf_2"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.cf.2.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "rabbitmq_0"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.rabbitmq.0.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "rabbitmq_1"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.rabbitmq.1.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "rabbitmq_2"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.rabbitmq.2.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "db_service_0"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.db_service.0.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "db_service_1"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.db_service.1.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}

resource "alicloud_snat_entry" "db_service_2"{
  snat_table_id = "${alicloud_nat_gateway.default.snat_table_ids}"
  source_vswitch_id = "${alicloud_vswitch.db_service.2.id}"
  snat_ip = "${alicloud_eip.default.ip_address}"
}
...
resource "alicloud_slb" "diego" {
  name = "diego_ssh_proxy${var.prefix}"
  internet_charge_type = "paybytraffic"
  internet=true
}

resource "alicloud_slb_listener" "tcp-2222" {
  load_balancer_id = "${alicloud_slb.diego.id}"
  backend_port = "2222"
  frontend_port = "2222"
  protocol = "tcp"
  bandwidth = 10
...
resource "alicloud_slb" "rabbitmq" {
  name = "rabbitmq_proxy${var.prefix}"
  internet_charge_type = "paybytraffic"
  internet=true
  specification="slb.s1.small"
}

resource "alicloud_slb_listener" "tcp-1883" {
  load_balancer_id = "${alicloud_slb.rabbitmq.id}"
  backend_port = "1883"
  frontend_port = "1883"
  protocol = "tcp"
  bandwidth = 10
...
resource "alicloud_slb_listener" "tcp-5671" {
  load_balancer_id = "${alicloud_slb.rabbitmq.id}"
  backend_port = "5671"
  frontend_port = "5671"
  protocol = "tcp"
...
resource "alicloud_slb_listener" "tcp-8883" {
  load_balancer_id = "${alicloud_slb.rabbitmq.id}"
  backend_port = "8883"
  frontend_port = "8883"
  protocol = "tcp"
...
resource "alicloud_slb_listener" "tcp-15672" {
  load_balancer_id = "${alicloud_slb.rabbitmq.id}"
  backend_port = "15672"
  frontend_port = "15672"
  protocol = "tcp"
...
resource "alicloud_slb_listener" "tcp-15674" {
  load_balancer_id = "${alicloud_slb.rabbitmq.id}"
  backend_port = "15674"
  frontend_port = "15674"
  protocol = "tcp"
...
resource "alicloud_slb_listener" "tcp-33672" {
  load_balancer_id = "${alicloud_slb.rabbitmq.id}"
  backend_port = "33672"
  frontend_port = "33672"
  protocol = "tcp"
...
resource "alicloud_slb_listener" "tcp-61613" {
  load_balancer_id = "${alicloud_slb.rabbitmq.id}"
  backend_port = "61613"
  frontend_port = "61613"
  protocol = "tcp"
...
resource "alicloud_instance" "default" {
  ...
  provisioner "local-exec" {
    command = <<EOF
        ...
        echo az1_vswitch_id_2: ${alicloud_vswitch.rabbitmq.0.id} >> ../group_vars/all
        echo az1_vswitch_id_3: ${alicloud_vswitch.db_service.0.id} >> ../group_vars/all

        echo az1_vswitch_range: ${alicloud_vswitch.cf.0.cidr_block} >> ../group_vars/all
        echo az1_vswitch_range_2: ${alicloud_vswitch.rabbitmq.0.cidr_block} >> ../group_vars/all
        echo az1_vswitch_range_3: ${alicloud_vswitch.db_service.0.cidr_block} >> ../group_vars/all

        echo az1_vswitch_gateway: ${element(var.cf_gateway, 0)} >> ../group_vars/all
        echo az1_vswitch_gateway_2: ${element(var.rabbitmq_gateway, 0)} >> ../group_vars/all
        echo az1_vswitch_gateway_3: ${element(var.db_service_gateway, 0)} >> ../group_vars/all
        ...
        echo az2_vswitch_id_2: ${alicloud_vswitch.rabbitmq.1.id} >> ../group_vars/all
        echo az2_vswitch_id_3: ${alicloud_vswitch.db_service.1.id} >> ../group_vars/all

        echo az2_vswitch_range: ${alicloud_vswitch.cf.1.cidr_block} >> ../group_vars/all
        echo az2_vswitch_range_2: ${alicloud_vswitch.rabbitmq.1.cidr_block} >> ../group_vars/all
        echo az2_vswitch_range_3: ${alicloud_vswitch.db_service.1.cidr_block} >> ../group_vars/all

        echo az2_vswitch_gateway: ${element(var.cf_gateway, 1)} >> ../group_vars/all
        echo az2_vswitch_gateway_2: ${element(var.rabbitmq_gateway, 1)} >> ../group_vars/all
        echo az2_vswitch_gateway_3: ${element(var.db_service_gateway, 1)} >> ../group_vars/all
        ...
        echo az3_vswitch_id_2: ${alicloud_vswitch.rabbitmq.2.id} >> ../group_vars/all
        echo az3_vswitch_id_3: ${alicloud_vswitch.db_service.2.id} >> ../group_vars/all

        echo az3_vswitch_range: ${alicloud_vswitch.cf.2.cidr_block} >> ../group_vars/all
        echo az3_vswitch_range_2: ${alicloud_vswitch.rabbitmq.2.cidr_block} >> ../group_vars/all
        echo az3_vswitch_range_3: ${alicloud_vswitch.db_service.2.cidr_block} >> ../group_vars/all

        echo az3_vswitch_gateway: ${element(var.cf_gateway, 2)} >> ../group_vars/all
        echo az3_vswitch_gateway_2: ${element(var.rabbitmq_gateway, 2)} >> ../group_vars/all
        echo az3_vswitch_gateway_3: ${element(var.db_service_gateway, 2)} >> ../group_vars/all

        echo tcp_slb_id_array: ${alicloud_slb.tcp.id} >> ../group_vars/all
        echo http_slb_id_array: ${alicloud_slb.http.id} >> ../group_vars/all
        echo diego_slb_id_array: ${alicloud_slb.diego.id} >> ../group_vars/all
        echo rabbitmq_slb_id_array: ${alicloud_slb.rabbitmq.id} >> ../group_vars/all
  EOF
  }
}
